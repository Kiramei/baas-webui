...
import { ControlMessage, KeyCodeControlMessage, ScrollControlMessage, TouchControlMessage } from './MessageCenter';
...
abstract class InteractionHandlerBase {
    ...
    protected static bindGlobalListeners(interactionHandler: InteractionHandlerBase): void {
        interactionHandler.touchEventsNames.forEach((eventName) => {
            let set: Set<InteractionHandlerBase> | undefined = InteractionHandlerBase.eventListeners.get(eventName);
            if (!set) {
                set = new Set();
                document.body.addEventListener(eventName, this.onInteractionEvent, InteractionHandlerBase.options);
        ...
        if (action === MotionEvent.ACTION_UP) {
            ...
            if (!previous) {
                if (
                    ...
                    const emulated = InteractionHandlerBase.createEmulatedMessage(MotionEvent.ACTION_DOWN, message);
    ...
    public drawPointer(point: Point): void {
        this.drawPoint(point, InteractionHandlerBase.touchPointRadius, InteractionHandlerBase.touchPointImage);
    ...
    public drawCenter(point: Point): void {
        this.drawPoint(point, InteractionHandlerBase.centerPointRadius, InteractionHandlerBase.centerPointImage);
    }
...
export class InteractionHandler extends InteractionHandlerBase {
    ...
    private onMouseLeave = (): void => {
        ...
        this.storedFromMouseEvent.forEach((message) => {
            this.listener.sendMessage(InteractionHandlerBase.createEmulatedMessage(MotionEvent.ACTION_UP, message));
        });
        this.storedFromMouseEvent.clear();
        this.clearCanvas();
    };
